...
float grid::ComputePhotonTimestepHeIII(float DensityUnits, float LengthUnits,
...
//    return huge_number;
  ...
  float HeIIIdot, a3inv, a6inv, kr5, kr6, cs;
...
//		   double(CellWidth[0][0]));
  ...
  for (k = GridStartIndex[2]; k <= GridEndIndex[2]; k++)
	...
	if (BaryonField[kphHeIINum][index] <= HeIIfront_kph &&
	    BaryonField[kphHeIINum][index] > tiny_number) {
...
// kr6 HeIII + e -> heII  + p
#ifdef USE_GRACKLE
	  if (grackle_data->use_grackle == TRUE) {
	    kr5 = grackle_rates.k5[tidx] + (logtem - t1) * 
	      (grackle_rates.k5[tidx+1] - grackle_rates.k5[tidx]) / tdef;
	    kr6 = grackle_rates.k6[tidx] + (logtem - t1) * 
	      (grackle_rates.k6[tidx+1] - grackle_rates.k6[tidx]) / tdef;
	  } else
#endif
	  {
	    kr5 = RateData.k5[tidx] + (logtem - t1) * 
	      (RateData.k5[tidx+1] - RateData.k5[tidx]) / tdef;
	    kr6 = RateData.k6[tidx] + (logtem - t1) * 
	      (RateData.k6[tidx+1] - RateData.k6[tidx]) / tdef;
	  }
	  HeIIIdot = a3inv *
	    (kr5 * BaryonField[HeIINum ][index] * BaryonField[DeNum][index]  -
	     kr6 * BaryonField[HeIIINum][index] * BaryonField[DeNum][index]) +
	     BaryonField[HeIINum][index] * BaryonField[kphHeIINum][index];

	  alldt[index] = MAX_CHANGE * BaryonField[HeIIINum][index] / HeIIIdot;
  ...
  if (DEBUG)
    if (imin != INT_UNDEFINED)
      printf("kpHeII=%g/%g, dt=%g, avg(dt)=%g, T=%g, HI=%g, HII=%g\n",
	     BaryonField[kphHeIINum][imin], HeIIfront_kph,
	     alldt[imin], dt, temperature[imin], 
	     BaryonField[HeIINum][imin] / BaryonField[0][imin],
	     BaryonField[HeIIINum][imin] / BaryonField[0][imin]);
